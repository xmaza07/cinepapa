...
export function addHapticToButtons(element: React.ReactNode): React.ReactNode {
  ...
  const children = React.Children.map(element.props.children, addHapticToButtons);

  // Check if this is a button or Button component
  const isButtonType = 
    element.type === 'button' || 
    (typeof element.type === 'function' && (
      ('displayName' in element.type && (element.type as { displayName?: string }).displayName === 'Button') ||
      ('name' in element.type && (element.type as { name?: string }).name === 'Button')
    ));
...
export function withHapticFeedback<P extends object>(
  ...
  if (typeof Component === 'function') {
    ...
    componentName = (Object.prototype.hasOwnProperty.call(Component, 'displayName')
