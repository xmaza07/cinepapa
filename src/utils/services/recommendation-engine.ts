...
class RecommendationEngine {
    ...
    const scores: RecommendationScore[] = await Promise.all(
      availableContent.map(async (media) => ({
        ...
        factors: {
          ...
          personalPreference: this.calculatePersonalPreferenceScore(media, userProfile),
  ...
  public async getSimilarContent(
    ...
    const similarities = await Promise.all(
      availableContent
        ...
        .map(async (media) => ({
          media,
          similarity: await nlpService.calculateSimilarity(referenceMedia, media)
        }))
    );
  ...
  public async processInteraction(
    ...
    const preferenceUpdates = await nlpService.processUserFeedback(interaction, media);
      ...
      switch (update.type) {
        case 'genre':
          userProfile.preferences.genreWeights[update.value] = 
            (userProfile.preferences.genreWeights[update.value] || 0) + update.weight;
          break;
        case 'actor':
          userProfile.preferences.actorWeights[update.value] = 
            (userProfile.preferences.actorWeights[update.value] || 0) + update.weight;
          break;
        case 'director':
          userProfile.preferences.directorWeights[update.value] = 
            (userProfile.preferences.directorWeights[update.value] || 0) + update.weight;
          break;
        case 'keyword':
          userProfile.preferences.keywords[update.value] = 
            (userProfile.preferences.keywords[update.value] || 0) + update.weight;
          break;
      }
    });
  ...
  private async calculateOverallScore(media: Media, userProfile: UserProfile): Promise<number> {
    const weights = {
      contentBased: 0.4,
      collaborative: 0.3,
  ...
  private async calculateContentBasedScore(media: Media, userProfile: UserProfile): Promise<number> {
    let score = 0;
    let weights = 0;

    // Genre matching
    if (media.genre_ids) {
      const genreScore = media.genre_ids.reduce((sum, genreId) => 
        sum + (userProfile.preferences.genreWeights[genreId.toString()] || 0), 
        0
    ...
    if (media.overview) {
      const overviewScore = Object.entries(userProfile.preferences.keywords)
    ...
    const mediaYear = new Date(media.release_date || media.first_air_date || '').getFullYear();
    if (mediaYear && userProfile.preferences.yearRange.start <= mediaYear && 
  ...
  private async calculateCollaborativeScore(media: Media, userProfile: UserProfile): Promise<number> {
    ...
    const userSimilarities = await Promise.all(
      similarUsers.map(async otherUser => {
        const commonInteractions = userProfile.interactions
          .filter(i1 => otherUser.interactions.some(i2 => i2.mediaId === i1.mediaId));

        if (commonInteractions.length === 0) return 0;

        const ratingCorrelation = commonInteractions.reduce((sum, i1) => {
          const i2 = otherUser.interactions.find(i => i.mediaId === i1.mediaId);
    ...
    const weightedScore = similarUsers.reduce((sum, user, index) => {
      ...
      return interaction ? sum + (interaction.rating / 5) * userSimilarities[index] : sum;
    }, 0);

    return weightedScore / similarUsers.length;
  }

  private calculatePersonalPreferenceScore(media: Media, userProfile: UserProfile): number {
    ...
    const similarToLiked = userProfile.interactions
      .filter(i => i.rating >= 4)
      .some(i => {
        const likedMedia = userProfile.watchHistory.find(m => m.id === i.mediaId);
        return likedMedia && media.genre_ids?.some(g => likedMedia.genre_ids?.includes(g));
      });
  ...
  private diversifyRecommendations(recommendations: Media[], userProfile: UserProfile): Media[] {
