...
export class RateLimiter {
  ...
  private constructor(private maxRequests: number, private windowMs: number) {
    this.limits = new Map();
  ...
  static getInstance(maxRequests: number = 100, windowMs: number = 60000): RateLimiter {
  ...
  async isAllowed(url: string, clientId: string): Promise<boolean> {
    ...
    const entry = entries.get(key) || { count: 0, resetTime: now + limit.windowMs };
