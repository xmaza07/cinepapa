...
const Hero = ({ media, className = '' }: HeroProps) => {
  ...
  // Get the first available media item with backdrop for Netflix-style single hero
  const featuredMedia = useMemo(() => {
    ...
    if (preference && preference !== 'balanced') {
      const preferred = withBackdrop.find(item => item.media_type === preference);
      if (preferred) return preferred as ExtendedMedia;
    }
    
    return withBackdrop[0] as ExtendedMedia;
  }, [media, preference]);
        ...
        <div className="max-w-screen-2xl mx-auto px-4 md:px-12 w-full">
          ...
          >
            ...
            <div className="flex items-center space-x-4 mb-4">
              <span className="netflix-text text-netflix-red font-bold text-sm uppercase tracking-wider">
                {featuredMedia.media_type === 'movie' ? 'Film' : 'Series'}
              </span>
              {releaseYear && (
                <span className="netflix-text text-netflix-light-gray text-sm">
                  {releaseYear}
                </span>
              )}
            ...
            {/* Title */}
            <h1 className="netflix-text text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4 leading-tight">
              {title}
            </h1>

            {/* Description */}
            <p className="netflix-text text-lg md:text-xl text-white mb-8 line-clamp-3 leading-relaxed max-w-md lg:max-w-lg">
